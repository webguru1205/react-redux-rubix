...
function tracksDeleteSuccess(trackId) {
    return {
...
function createTrack(projectId, file) {
    ...
    const metadata = [{
        title: 'Untitled', filename: file.filename, size: file.size, mimetype: file.mimetype
    }];

    return sendApiRequest({
        method: 'POST',
        url: 'projects/' + projectId + '/tracks/',
        data: { project: projectId, user: userId, audio_file: file.url, metadata: JSON.stringify(metadata)},
