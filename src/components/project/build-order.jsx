    ...
    return (
      <li>
        <Row>
          ...
          <Col md={7}>
            <span className="track-title">{ metadata.title ? metadata.title : '---- ----' }</span>
            <span className="artist">{ metadata.artist? metadata.artist: '----' }</span>
...
class ProjectBuildOrder extends React.Component {
  constructor(...args) {
    ...
    this.state = { projectId: projectId, tracks: [], credit: 1, availableCredts: 0 };
  }
  ...
  buildProcess() {
    ...
    tracks.forEach(function(track) {
      dispatch(actions.buildTrack(project, artworks[0], track, size, totalCredits));
    });
            ...
            <Row>
              <Col sm={12}>
                <Button lg bsStyle='primary' className="btn-text-large btn-block"
                  disabled={ disabled } onClick={::this.buildProcess}>Place Your Order</Button>
              </Col>
            </Row>
          </div>
