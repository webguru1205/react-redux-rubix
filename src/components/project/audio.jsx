...
class TrackEdit extends React.Component {
  ...
  submit(e) {
    ...
    track.metadata = JSON.stringify([metadata]);
...
class ProjectAudio extends React.Component {
  constructor(...args) {
    ...
    this.state = {
      ...
      drag: false, playingTrack: {}, playingStatus: null
    };
  }
  ...
  componentWillReceiveProps (nextProps) {
    ...
    const { status, fileId, removedTrackId } = nextProps;
    ...
    if(status == 'deleted') {
      if(playingTrack && playingTrack.id == removedTrackId) {
  ...
  render() {
    ...
    const { projectId, editModal, editTrack, drag, tracks, playingTrack } = this.state;
    console.log("On audio track page, loading is: ", loading);
    let alert = null;
    let isLoading = true;
              ...
              {((thisInstance) => {
                  if(!isLoading) {
                    if(tracks.length) {
                      ...
                      return <tr><td colSpan={6} className="text-danger text-center">No tracks were found.</td></tr>;
                    }
                  } else {
                    return <tr><td colSpan={6} className="text-center"><div className="loading"></div></td></tr>;
                  }
                })(this)
              }
              </thead>
...
const mapStateToProps = (state) => ({
  ...
  removedTrackId: state.tracks.removedTrackId,
